...
//
    function calculateXAndY(){
        ...
        if (isValidInput(value_X,value_Y)){
            p_result.textContent="Old Value is " + String(calculateOldValue(Number(value_X),Number(value_Y)));
            p_result_incremental_rate.textContent="Incremental rate is " + String(calculateIncrementalRateBetweenXAndY(Number(value_X),Number(value_Y)))+" %";  
        }else
        {
